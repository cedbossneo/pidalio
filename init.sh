...
if [[ "$EXISTING_ETCD" == "" ]]
then
    docker run -e TOKEN=${TOKEN} -e OS_USERNAME=${OS_USERNAME} -e OS_PASSWORD=${OS_PASSWORD} -e OS_AUTH_URL=${OS_AUTH_URL} -e OS_TENANT_NAME=${OS_TENANT_NAME} -e OS_REGION_NAME=${OS_REGION_NAME} --rm --name=etcd -p 2379:2379 -p 2380:2380 cedbossneo/docker-etcd-rclone
else
